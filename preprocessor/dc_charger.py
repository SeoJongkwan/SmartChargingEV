...
InDeokWonIT_grouped = InDeokWonIT_dc_charger.groupby(['month', select_period])
...
InDeokWonIT_stat['charging_time'] = list(InDeokWonIT_grouped['charging_time'].sum())
InDeokWonIT_stat['occupation'] = round(InDeokWonIT_stat['charging_time'].apply(lambda x: x / (24 * 60 * 60) * 100),2)
